...
```{r}
lsmeans_ne1 <- emmeans(model_ne, ~ solution * nozzle * sprayer, cont="pairwise", 
...
```{r}
plot(lsmeans_ne1$emmeans, ~ solution * nozzle * sprayer, comparisons=TRUE, 
...
```{r}
library(multcomp)
cld_ne <- cld(lsmeans_ne1$emmeans, alpha=0.05, Letters=letters, adjust="none", reversed = TRUE)
cld_ne
```


```{r}
ne_insw <- as_tibble(cld_ne) %>% 
...
```{r}
ne_lsmeans <- emmeans(ne_model, ~ solution*sprayer*nozzle, cont="pairwise", type = "response",
                      adjust="none", alpha=0.05)
...
```{r}
plot(ne_lsmeans$emmeans, ~ solution*sprayer*nozzle, comparisons=TRUE, alpha=0.05, type = "response",
     adjust="none") +
...
```{r}
cld_neup <- cld(ne_lsmeans$emmeans, alpha=0.05, Letters=letters, adjust="none", reversed = TRUE)
cld_neup
```


```{r}
ne_up <- as_tibble(cld_neup) %>% 
  unite("trt", c("solution", "sprayer", "nozzle"), sep = " ", remove = FALSE)
ne_up 
```
...
up_ne <- ne_up %>% 
  ...
  geom_linerange(aes(ymin = lower.CL, ymax = upper.CL), size = 1.5) +
  coord_flip() +
  geom_text(nudge_y = c(0.15, 0.15, 0.13, 0.13, 0.13, 0.13,
                         0.04, 0.04, 0.03, 0.03, 0.03, 0.03)) +
  scale_color_brewer(palette = "Paired") +
  geom_text(aes(x = 1.5, y = 0.2,
                label = "Solution makes a bigger difference"),
...
mean_pred <-  exp(lme4:::predict.merMod(ne$model[[3]], dat, 
                                        re.form = NA))
...
#  ylim(0,100) +
 scale_color_brewer(palette = "Paired") +
  geom_text(nudge_y = c(55, 50, 48, 35, 30, 27,  
                        28, 18, 13, 10, 8, 8)) +
  theme_test() + 
...
data <- weather %>% 
  ...
  rename(dir = wind_dir_avg,
         spd = wind_sp_avg)
write_csv(file = data, file = "wind.csv")
```
...
plot <- plot.windrose(data = data,
              spd = "spd",
              dir = "dir") 
ne_speed <- plot +  #facet_wrap(~location) +  
    ggsave("WindRose_NE.png", units="in")
```
...
```{r}
all_wi <- (ne_speed + ins_ne) / figs_ne / (auc_ne | c) +
   plot_layout(heights =  c(1, 2, 1)) +
  plot_annotation(title = "<img src='https://upload.wikimedia.org/wikipedia/commons/thumb/e/e5/Nebraska_Cornhuskers_logo.svg/171px-Nebraska_Cornhuskers_logo.svg.png' width='60'/>") 

ggsave("nerbaska.pdf", all_wi, width = 9, height = 12)
```
