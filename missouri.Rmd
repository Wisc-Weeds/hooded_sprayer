...
```{r}
model <- function(df){
  lmer(deposition ~  sprayer * nozzle + (1|block), 
              REML = FALSE, data = df)
}
```
...
```{r}
lsmeans_mo <- emmeans(model_mo, ~  nozzle * sprayer, cont="pairwise", 
...
```{r}
plot(lsmeans_mo$emmeans, ~ nozzle * sprayer, comparisons=TRUE, 
     alpha=0.05, adjust="none") 
```


```{r}
cld_mo <- cld(lsmeans_mo$emmeans, alpha=0.05, Letters=letters, adjust="none", reversed = TRUE)
cld_mo
```


```{r}
mo_insw <- as_tibble(cld_mo) %>% 
  unite("trt", c("sprayer", "nozzle"), sep = " ", remove = FALSE)
mo_insw 
```
...
ins_mo <- mo_insw  %>% 
  ...
  coord_flip() +
  geom_text(nudge_y = 220) +
  geom_text(aes(x = 5, y = 550,
...
```{r}
mo_lsmeans <- emmeans(mo_model, ~ sprayer*nozzle, cont="pairwise", type = "response",
                      adjust="none", alpha=0.05)
...
```{r}
plot(mo_lsmeans$emmeans, ~ sprayer*nozzle, 
     comparisons=TRUE, alpha=0.05, type = "response",
     adjust="none") 
```
...
```{r}
cld_moup <- cld(mo_lsmeans$emmeans, alpha=0.05, Letters=letters, adjust="none", reversed = TRUE)
cld_moup
```


```{r}
mo_up <- as_tibble(cld_moup) %>% 
  unite("trt", c("sprayer", "nozzle"), sep = " ", remove = FALSE)
mo_up 
```
...
mean_pred <-  exp(lme4:::predict.merMod(mo$model[[3]], dat, 
                                        re.form = NA))
...
fig1 <- fig %>% 
  ...
  unnest(estimates) %>% 
  rename(distance_m = x) %>% 
  unite("trt", c("sprayer", "nozzle"), sep = " ") %>% 
  mutate_if(is_character, as_factor) 
```
...
#  background_grid() +
  theme(legend.position = "none") +
  geom_label(aes(label = formula), x = 30, y = 9, color = 1) +
...
#  ylim(0,100) +
 scale_color_brewer(palette = "Paired") +
  geom_text(nudge_y = c(13, 10, 7, 7, 5, 5)) +
  theme_test() + 
  geom_text(aes(x = 2, y = 30,
                label = "Lower values\nless spray \nparticle\n deposition\n from 1 to 60 m"),
            stat = "unique", family = "flower",
            size = 4, color = "darkcyan") +
  labs(title = "Area under the curve at downwind",
...
plot <- plot.windrose(data = data,
              spd = "spd",
              dir = "dir") 
mo_speed <- plot +  #facet_wrap(~location) +  
    ggsave("WindRose_MO.png", units="in")
```
...
```{r}
all_wi <- (mo_speed + ins_mo) / figs_mo / (auc_mo | c) +
   plot_layout(heights =  c(1, 2, 1)) +
  plot_annotation(title = "<img src='https://tirecovers.com/pub/media/wysiwyg/College_logos/Missouri.jpg' width='60'/>") 

ggsave("missouri.pdf", all_wi, width = 9, height = 12)
```
