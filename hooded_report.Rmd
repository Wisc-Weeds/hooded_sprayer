...
a <- downwind_final %>% 
  ...
  coord_flip() +
  geom_text(aes(x = 1, y = 25,
                label = "Lower values lower deposition"),
            stat = "unique", family = "bell",
            size = 4, color = "darkcyan") +
  labs(title = "Intercept at downwind", 
       y = expression(paste("Solution deposition (\u03b7L/cm"^"2",")")), x = "") +
  theme_test() +
  theme(legend.position = "none",
...
b <- downwind_final %>% 
  ...
  geom_linerange(aes(ymin = -lower, ymax = -upper), size = 1.5) +
  coord_flip() +
  geom_text(aes(x = 12, y = -0.43,
                label = "Higher values lower deposition rate"),
            stat = "unique", family = "bell",
            size = 4, color = "darkcyan") +
...
c <- distance_final1 %>% 
  ...
  geom_text(aes(x = 2, y = 80,
                label = "Distance where dicamba \nreached 0 deposition"),
            stat = "unique", family = "bell",
            size = 4, color = "darkcyan") +
  labs(title = "Predicted downwind distance", 
       y = "Distance (m)", x = "") +
...
ggplot(aes(x=reorder(trt, response), 
               y=response, color=trt)) + 
  geom_point(size = 2, stroke = 1) +
#  ylim(0,100) +
 scale_color_brewer(palette = "Paired") +
  theme_test() + 
  geom_text(aes(x = 1.5, y = 90,
                label = "Lower values less dicamba deposition \nfrom 1 to 60 m"),
            stat = "unique", family = "bell",
            size = 4, color = "darkcyan") +
  labs(title = "Area under the curve at downwind",
       y= "Area under the curve", x="") +
